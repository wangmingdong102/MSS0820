...
/* mss_lcdtest_movietest_2017_10_15 */
/* -2014-11-27 16:15:17 */
        registerDTMFToneReceiver(context);
/* -2014-11-27 16:15:17 */
/* mss_lcdtest_movietest_2017_10_15 */
    }
    ...
    public void updateSettings() {
        ...
        synchronized (mLock) {
            ...
            /* mss_lcdtest_movietest_2017_10_15 */
            mLockScreenTimeout = -1;//Settings.System.getIntForUser(resolver,
...
/* mss_lcdtest_movietest_2017_10_15 */
/* mss-2015-7-5 0:6:16 */
        Log.d("jdj-key", " keyCode:"+keyCode);

        if(keyCode == KeyEvent.KEYCODE_VOLUME_DOWN){
            if(down && (event.getFlags() & KeyEvent.FLAG_LONG_PRESS) != 0){
                Log.d("jdj-key", " FLAG_LONG_PRESS keyCode:"+keyCode);
                if (SystemProperties.get("persist.sys.mssmode", "1").equals("1")) {
                    if (!isOTPOnTop(mContext)) {
                        if (isTestMode(mContext)) {
                            Log.d("jdj-key", " isTestMode");
                            Intent mltstop = new Intent("action.cxb.stop");
                            mContext.sendBroadcast(mltstop);
                            playTone(ToneGenerator.TONE_DTMF_0, 200);
                        } else {
                            Intent mltstartactivity = new Intent();
                            mltstartactivity.setClassName("com.xxd.android", "com.xxd.android.XXDLcdTest");
                            mltstartactivity.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                            mContext.startActivity(mltstartactivity);
                            playTone(ToneGenerator.TONE_DTMF_0, 200);
                        }
                        return -1;
                    }
                }else if (SystemProperties.get("persist.sys.mssmode", "1").equals("2")) {
                    if (!isOTPOnTop(mContext)) {
                        if (isMoviePlaying(mContext)) {
                            Log.d("jdj-key", " isMoviePlaying");
                            Intent mltstop = new Intent("action.cxb.stopmovie");
                            mContext.sendBroadcast(mltstop);
                            playTone(ToneGenerator.TONE_DTMF_0, 200);
                        } else {
                            movieAutoPlayHandler();
                        }
                        return -1;
                    }
                }
        ...
        if(keyCode == KeyEvent.KEYCODE_VOLUME_UP){
            if(down && (event.getFlags() & KeyEvent.FLAG_LONG_PRESS) != 0){
                Log.d("jdj-key", " FLAG_LONG_PRESS keyCode:"+keyCode);
                if(!isTestMode(mContext)){
                    if (SystemProperties.get("persist.sys.otp", "1").equals("2")) {
                        OTPHandler();
                    }
                    return -1;
                }
...
/* mss-2015-7-9 0:11:31 */

        if(keyCode == KeyEvent.KEYCODE_BACK){
            Log.d("jdj-key", "KEYCODE_BACK down:"+down);
            if(down && (event.getFlags() & KeyEvent.FLAG_LONG_PRESS) != 0){
                if(!isTestMode(mContext) && !isOTPOnTop(mContext)){
                    movieAutoPlayHandler();
                    return -1;
                }
/*                if(isTestMode(mContext)){
...
/* mss_lcdtest_movietest_2017_10_15 */
        Log.d("jdj-key", "interceptKeyBeforeQueueing keyCode:"+keyCode);
/* irb 2015-7-1 10:22:14 */
        if (SystemProperties.get("persist.sys.scdr", "1").equals("2")) {
            if ((keyCode == KeyEvent.KEYCODE_VOLUME_DOWN
                    || keyCode == KeyEvent.KEYCODE_VOLUME_UP) && !down && isTestMode(mContext)) {
                synchronized (mScreenCurrentDisplayRunnable) {
                    mHandler.removeCallbacks(mScreenCurrentDisplayRunnable);
                    mHandler.post(mScreenCurrentDisplayRunnable);
                }
            }
        }
/*
        if(keyCode == KeyEvent.KEYCODE_VOLUME_DOWN
            ...
            if (isTestMode(mContext)
                    || isLauncher(mContext)
                    || isMoviePlaying(mContext)
                    || isMovieOnTop(mContext)) {
...
/* -2014-11-27 16:39:43 */
        if(down){
            Intent intent = new Intent("cxb.intent.keydown");
            intent.putExtra("EXTRA_KEY_CODE", keyCode);
            mContext.sendBroadcast(intent);
        }

        if(keyCode == KeyEvent.KEYCODE_HOME){
            Log.d("jdj-key", "KEYCODE_HOME ");

            Log.d("jdj-dtmf", "interceptKeyBeforeQueueing not isAtHome");
            if(isTestMode(mContext)
                    || isOTPOnTop(mContext)){
                Log.d("jdj-dtmf", "interceptKeyBeforeQueueing isTestMode");
                return 0;
            }
...
/* mss_lcdtest_movietest_2017_10_15 */
    ...
    private void dispatchDirectAudioEvent(KeyEvent event) {
        ...
        Log.d(TAG, "dispatchDirectAudioEvent pkgName:"+pkgName);
        switch (keyCode) {
            case KeyEvent.KEYCODE_VOLUME_UP:
                try {
...
/* mss_lcdtest_movietest_2017_10_15 */
    /* -2014-11-27 16:8:20 */
    private final BroadcastReceiver mDTMFToneReceiver = new DTMFToneBroadcastReceiver();
    private ToneGenerator mToneGenerator;
    private final Object mToneGeneratorLock = new Object();

    private void playTone(int tone, int durationMs) {
/* jdj-tieto-2014-11-27 16:9:39
 ...
 jdj-tieto-2014-11-27 16:9:39 */
        ...
        synchronized (mToneGeneratorLock) {
            if (mToneGenerator == null) {
                Log.w("jdj-dtmf", "playTone: mToneGenerator == null, tone: " + tone);
                return;
            }
    ...
    private void stopTone() {
        ...
        synchronized (mToneGeneratorLock) {
            if (mToneGenerator == null) {
                Log.w("jdj-dtmf", "stopTone: mToneGenerator == null");
                return;
            }
...
/* jdj-tieto-2014-11-27 16:11:17
 ...
 jdj-tieto-2014-11-27 16:11:17 */
    ...
    private void playDTMFTone(String tone){
        int toneItem = -1;
        for(int index = 0; index < 12; index++){
            ...
            if(index == 11)
                ...
                value +=  index;
            if(tone != null && tone.equals(value)){
                toneItem = index;
                break;
            }
        }
        Log.d("jdj-dtmf", "playDTMFTone tone:"+tone);
        Log.d("jdj-dtmf", "playDTMFTone toneItem:"+toneItem);

        if(toneItem != -1)
            playTone(ToneGenerator.TONE_DTMF_0 + toneItem, 300);
    }

    private void registerDTMFToneReceiver(Context context) {
        IntentFilter toneIntentFilter = new IntentFilter();
        toneIntentFilter.addAction("action.cxb.dtmf");
        toneIntentFilter.addAction("action.cxb.delay");
        toneIntentFilter.addAction("action.cxb.start");
/*        toneIntentFilter.addAction("action.rmc.init");
        toneIntentFilter.addAction("action.rmc.start");
        toneIntentFilter.addAction("action.rmc.stop");
        toneIntentFilter.addAction("action.rmc.showtouch");*/

/* mss-2015-7-7 0:38:12 */
        toneIntentFilter.addAction("action.otp.sleep");
        toneIntentFilter.addAction("action.otp.wakeup");
/* mss-2015-7-7 0:38:12 */


        context.registerReceiver(mDTMFToneReceiver, toneIntentFilter);

        synchronized (mToneGeneratorLock) {
            if (mToneGenerator == null) {
                try {
                    mToneGenerator = new ToneGenerator(AudioManager.STREAM_DTMF, /*TONE_RELATIVE_VOLUME*/80);
                } catch (RuntimeException e) {
    ...
    private class DTMFToneBroadcastReceiver extends BroadcastReceiver {
        @Override
        public void onReceive(Context context, Intent intent) {
            String action = intent.getAction();
            Log.d("jdj-dtmf", "DTMFToneBroadcastReceiver action:"+action);
            if ("action.cxb.dtmf".equals(action)) {
                String dtmf_key = intent.getStringExtra("dtmf_key");//0-9 * #
                Log.d("jdj-dtmf", "DTMFToneBroadcastReceiver dtmf_key:"+dtmf_key);
                stopTone();
                playDTMFTone(dtmf_key);
            }else if("action.cxb.delay".equals(action)) {
                int delay = intent.getIntExtra("delay", 1000);
                Log.d("jdj-dtmf", "DTMFToneBroadcastReceiver delay:"+delay);
                SystemProperties.set("persist.sys.mssdelay", "" + delay);

                Intent delayset = new Intent("action.cxb.delayset");
                mContext.sendBroadcast(delayset);
            }else if("action.cxb.start".equals(action)){
                Intent mltstartactivity = new Intent();
                mltstartactivity.setClassName("com.xxd.android", "com.xxd.android.XXDLcdTest");
                mltstartactivity.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                mContext.startActivity(mltstartactivity);
            }else if("action.rmc.init".equals(action)){
/*                Intent preparation = new Intent();
                preparation.setClassName("com.android.settings", "com.android.settings.MLPreparation");
                preparation.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                mContext.startActivity(preparation);*/
            }else if("action.rmc.showtouch".equals(action)){
/*                String status = intent.getStringExtra("status");
                if(status != null && status.equals("start")){
                    Settings.System.putInt(mContext.getContentResolver(), Settings.System.POINTER_LOCATION, 1);
                    Settings.System.putInt(mContext.getContentResolver(), Settings.System.SHOW_TOUCHES, 1);
                }else{
                    Settings.System.putInt(mContext.getContentResolver(), Settings.System.POINTER_LOCATION, 0);
                    Settings.System.putInt(mContext.getContentResolver(), Settings.System.SHOW_TOUCHES, 0);
                }*/
            }
/* jdj-tieto-2014-12-1 21:47:36
		else if("action.rmc.start".equals(action)){
			Intent rmcstart = new Intent();
			rmcstart.setClassName("org.onaips.vnc", "org.onaips.vnc.MainActivity");
			rmcstart.putExtra("status", "start");
			rmcstart.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
			mContext.startActivity(rmcstart);

		}else if("action.rmc.stop".equals(action)){
			Intent rmcstop = new Intent();
			rmcstop.setClassName("org.onaips.vnc", "org.onaips.vnc.MainActivity");
			rmcstop.putExtra("status", "stop");
			rmcstop.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
			mContext.startActivity(rmcstop);

		}
 jdj-tieto-2014-12-1 21:47:36 */

/* mss-2015-7-7 0:38:55 */
            else if("action.otp.sleep".equals(action)){
                if(mPowerManager != null)
                    mPowerManager.goToSleep(SystemClock.uptimeMillis());

            }else if("action.otp.wakeup".equals(action)){
                if(mPowerManager != null)
                    mPowerManager.wakeUp(SystemClock.uptimeMillis());


/* mss-2015-7-7 0:58:50
			Intent otpreadynotificationintent = new Intent("otpreadynotification");

			mContext.sendBroadcast(otpreadynotificationintent);
 mss-2015-7-7 0:58:50 */
            }
/* mss-2015-7-7 0:38:55 */
...
/* -2014-11-27 16:8:20 */
    ...
    /* mss-2015-3-18 20:51:26 */
    private boolean isOTPOnTop(Context context){
        ActivityManager am = (ActivityManager)context.getSystemService(Context.ACTIVITY_SERVICE);
        ComponentName cn = am.getRunningTasks(1).get(0).topActivity;
        return "com.mss.otp".equals(cn.getPackageName());
    }

    private static boolean mVUPPressed = false;
    private static boolean mVDOWNPressed = false;
    private static boolean mOTPEntered = false;

    private void OTPHandler(){
        if(isOTPOnTop(mContext))
            return;

/* mss-2015-7-3 0:36:35
        IAudioService audioService = getAudioService();
        if (audioService != null) {
	        try {
	            audioService.forceDismissVolumeDialog();
	        } catch (RemoteException e) {
	            Log.w(TAG, "IAudioService.forceDismissVolumeDialog() threw RemoteException " + e);
	        }
        }
 mss-2015-7-3 0:36:35 */
...
/* mss-2015-4-21 0:7:7 */
        otpIntent.setClassName("com.android.settings", "com.android.settings.OTPPreparation");
/* mss-2015-4-21 0:7:7 */
/* mss-2015-4-21 0:4:19
	otpIntent.setClassName("com.mss.otp", "com.mss.otp.OTPMainActivity");
 mss-2015-4-21 0:4:19 */
        otpIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        mContext.startActivity(otpIntent);
        Log.d("jdj-otp", "startActivity OTP");
    }
...
/* mss-2015-3-18 20:51:26 */
    ...
    private boolean isTestMode(Context context){
        ActivityManager am = (ActivityManager)context.getSystemService(Context.ACTIVITY_SERVICE);
        ComponentName cn = am.getRunningTasks(1).get(0).topActivity;
        Log.d("jdj-movie", "isTestMode class"+cn.getClassName());
        return "com.xxd.android".equals(cn.getPackageName())
                && cn.getClassName().equals("com.xxd.android.XXDLcdTest");
    }
    /*
        private boolean isMovieActivity(Context context){
            ActivityManager am = (ActivityManager)context.getSystemService(Context.ACTIVITY_SERVICE);
            ComponentName cn = am.getRunningTasks(1).get(0).topActivity;
            return "com.android.gallery3d".equals(cn.getPackageName()) && "com.android.gallery3d.app.MovieActivity".equals(cn.getClassName());
        }
    */
...
/* -2014-11-27 16:39:49 */
/*
    private static boolean mHomeKeyPressed = false;
    private static boolean mVUKeyPressed = false;
    private static boolean mVDKeyPressed = false;
    private static boolean mMLTStart = false;
    private static boolean mMLTStop = false;
*/

    private static boolean mMenuPressed = false;
    private static boolean mBackPressed = false;
    private static boolean mMovieEntered = false;

    /* mss-2015-7-4 23:45:44 */
    private boolean isMovieOnTop(Context context){
        ActivityManager am = (ActivityManager)context.getSystemService(Context.ACTIVITY_SERVICE);
        ComponentName cn = am.getRunningTasks(1).get(0).topActivity;
        Log.d("jdj-movie", "isMovieOnTop class"+cn.getClassName());
        return "com.xxd.android".equals(cn.getPackageName())
                && cn.getClassName().equals("com.xxd.android.MovieTest");
    }
    private boolean isMoviePlaying(Context context){
        ActivityManager am = (ActivityManager)context.getSystemService(Context.ACTIVITY_SERVICE);
        ComponentName cn = am.getRunningTasks(1).get(0).topActivity;
        Log.d("jdj-movie", "isMoviePlaying class"+cn.getClassName());
        return "com.android.gallery3d".equals(cn.getPackageName())
                && cn.getClassName().equals("com.android.gallery3d.app.MovieActivity");
    }
/* mss-2015-7-4 23:45:44 */

    private void movieAutoPlayHandler(){

        Log.d("jdj-movie", "movieAutoPlayHandler");

/* mss-2015-7-4 23:46:12 */
        if(isMovieOnTop(mContext)){
            return;
        }
/* mss-2015-7-4 23:46:12 */

        Intent movieIntent = new Intent();
        movieIntent.setClassName("com.xxd.android", "com.xxd.android.MovieTest");
        movieIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        mContext.startActivity(movieIntent);
        Log.d("jdj-movie", "startActivity MovieTest");
    }


    /* irb 2015-7-1 10:4:8 */
    private final static String SUBLCD_DRIVER_FILE = "/sys/devices/platform/autoload/driver/send";
    private final static String OTP_DRIVER_FILE = "/sys/devices/platform/autoload/driver/recv";
    //private final static String SUBLCD_DRIVER_FILE = "/sys/devices/platform/sublcd/driver/send";
    //private final static String OTP_DRIVER_FILE = "/sys/devices/platform/sublcd/driver/recv";
    private final static String SUBLCD_ID_REQUEST_CODE = "aa_55_01";
    private final static String SUBLCD_CURRENT_REQUEST_CODE = "aa_55_02";

    private ScreenCurrentDisplayRunnable mScreenCurrentDisplayRunnable = new ScreenCurrentDisplayRunnable();
    class ScreenCurrentDisplayRunnable implements Runnable {
        @Override
        public void run() {
            synchronized (this) {
                ScreenCurrentDisplayRequest();
            }
        }
    }
    private void ScreenCurrentDisplayRequest(){
        writeToFile(SUBLCD_CURRENT_REQUEST_CODE);
    }

    private ScreenIdDisplayRunnable mScreenIdDisplayRunnable = new ScreenIdDisplayRunnable();
    class ScreenIdDisplayRunnable implements Runnable {
        @Override
        public void run() {
            synchronized (this) {
                ScreenIdDisplayRequest();
            }
        }
    }
    private void ScreenIdDisplayRequest(){
        writeToFile(SUBLCD_ID_REQUEST_CODE);
    }

    private OtpReadyReadRunnable mOtpReadyReadRunnable = new OtpReadyReadRunnable();
    class OtpReadyReadRunnable implements Runnable {
        @Override
        public void run() {
            synchronized (this) {
                OtpReadyReadRequest();
    ...
    private void OtpReadyReadRequest(){
/* mss-2015-7-7 0:46:52
	  String data = readFileData(OTP_DRIVER_FILE);
  	int dataInt = 0;
	  if(data != null){
		try{
	...
	Intent intent = new Intent("otpreadynotification");
	intent.putExtra("otpreadyvalue", dataInt);
	mContext.sendBroadcast(intent);

	Log.d(TAG, "OtpReadyReadRequest dataInt:"+dataInt);
 mss-2015-7-7 0:46:52 */
    }

    /* mss-2015-7-28 22:14:47 */
    private StartLCDPreloadRunnable mStartLCDPreloadRunnable = new StartLCDPreloadRunnable();
    class StartLCDPreloadRunnable implements Runnable {
        @Override
        public void run() {
            synchronized (this) {
                StartLCDPreloadRequest();
            }
        }
    }
    private void StartLCDPreloadRequest(){
        try{
            Intent mltstartactivity = new Intent();
            mltstartactivity.setClassName("com.mss.lcd", "com.mss.lcd.LcdCFGPreload");
            mltstartactivity.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            mContext.startActivityAsUser(mltstartactivity, new UserHandle(UserHandle.USER_CURRENT));
        }catch(Exception e){
...
/* mss-2015-7-28 22:14:47 */
    ...
    public void writeFileData(String fileName,String data){
        Log.d(TAG, "writeFileData fileName:"+fileName);
        Log.d(TAG, "writeFileData data:"+data);
        ...
        try{
            ...
            fout.write(bytes);
            Log.d(TAG, "writeFileData write");
        } catch(Exception e){
            e.printStackTrace();
            Log.d(TAG, "writeFileData e.toString():"+e.toString());
/* mss-2015-7-3 0:3:12
			Toast.makeText(mContext, "Error code: 4", Toast.LENGTH_SHORT).show();
 mss-2015-7-3 0:3:12 */
            Log.d(TAG, "Error code: 4");
        }finally{
            if (fout != null) {
                try {
                    fout.close();
    ...
    private void writeToFile(String otpStr){
        if(otpStr == null)
            return;
        File mFile = new File(SUBLCD_DRIVER_FILE);
        if(mFile != null ){
            if(mFile.exists()){
                if(!mFile.isDirectory()){
                    writeFileData(mFile.getAbsolutePath(), otpStr);
                }else{
/* mss-2015-7-3 0:3:19
					Toast.makeText(mContext, "Error code: 1", Toast.LENGTH_SHORT).show();
 mss-2015-7-3 0:3:19 */
                    Log.d(TAG, "Error code: 1");
                }
            }else{
/* mss-2015-7-3 0:3:20
				Toast.makeText(mContext, "Error code: 2", Toast.LENGTH_SHORT).show();
 mss-2015-7-3 0:3:20 */
                Log.d(TAG, "Error code: 2");
            }
        }else{
/* mss-2015-7-3 0:3:22
			Toast.makeText(mContext, "Error code: 3", Toast.LENGTH_SHORT).show();
 mss-2015-7-3 0:3:22 */
            Log.d(TAG, "Error code: 3");
        }
    }

    public String readFileData(String fileName){
        Log.d(TAG, "readFileData fileName:"+fileName);
        String data = null;
        ...
        try {
            ...
            data = null;
/* mss-2015-7-3 0:3:42
			Toast.makeText(mContext, "Error code: 5", Toast.LENGTH_SHORT).show();
 mss-2015-7-3 0:3:42 */
            Log.d(TAG, "Error code: 5");
        }
        finally {
            try {
                br.close();
                ...
                data = null;
/* mss-2015-7-3 0:3:49
			Toast.makeText(mContext, "Error code: 6", Toast.LENGTH_SHORT).show();
 mss-2015-7-3 0:3:49 */
                Log.d(TAG, "Error code: 6");
            }
        }
        Log.d(TAG, "readFileData data:"+data);
        return data;
    }
