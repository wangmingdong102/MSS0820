...
import com.android.launcher3.mss.PageViewAnimation;
...
import static com.android.launcher3.mss.PageViewAnimation.PAGEVIEW_ANIMATION_FIRST;
import static com.android.launcher3.mss.PageViewAnimation.PAGEVIEW_ANIMATION_LAST;
import static com.android.launcher3.mss.PageViewAnimation.PAGEVIEW_ANIMATION_ZOOM_IN;
...
public class Launcher extends Activity
    ...
    /// M. ALPS02295402, add this for RtoL layout.
    ...
    public static boolean sUsePendingAppsQueue = false;  //op09 private

    /// M: list used to store all pending add/update/removed applications.
    public static ArrayList<PendingChangedApplications> sPendingChangedApps =
                            new ArrayList<PendingChangedApplications>();  //op09 private
    ...
    static final String PAGEVIEWANIMATION_TYPE = "launcher.pageview_animation_type";/* mssflexiblescreen */
...
/* mss_lcdtest_movietest_2017_10_15 */
        ...
        /* mssflexiblescreen */
        //setPageViewAnimaType(PAGEVIEW_ANIMATION_FIRST);
        int id = mSharedPrefs.getInt(PAGEVIEWANIMATION_TYPE, 0);
    ...
    private void handleActivityResult(
        ...
        //M:[OP09] handle hide apps @{
           ...
           //op09  mAppsCustomizeContent.mViewPluginEx.processAppsStateChanged();
    ...
    protected void onStop() {
        ...
        if (powerManager.isScreenOn()) {
            if (LOGD) {
                LauncherLog.d(TAG, "(Launcher)onStop: PowerManager.isScreenOn() = fasle,"
                    + " don't set SharedPreferences");
            }
    ...
    protected void onResume() {
        ...
        final View overview_panel_root = findViewById(R.id.overview_panel_root);
        final View switch_animtaion_list_root = findViewById(R.id.switch_animtaion_list_root);
        if(switch_animtaion_list_root.getVisibility() == View.VISIBLE){
            switch_animtaion_list_root.setVisibility(View.GONE);
            overview_panel_root.setVisibility(View.VISIBLE);
        }
    }
    ...
    private void setupViews() {
        ...
        int id = mSharedPrefs.getInt(PAGEVIEWANIMATION_TYPE, 0);
        TextView switchAnimation = (TextView)findViewById(R.id.switch_animtaion);
        switchAnimation.setTag(id);
        switchAnimation.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                overview_panel_root.setVisibility(View.GONE);
                switch_animtaion_list_root.setVisibility(View.VISIBLE);

                LinearLayout switch_animtaion_list = (LinearLayout)findViewById(R.id.switch_animtaion_list);

                if(switch_animtaion_list.getChildCount() < 1) {
                    Drawable drawableTop = getResources().getDrawable(
                            R.drawable.ic_effect);
                    final int currentAnimationId = mSharedPrefs.getInt(PAGEVIEWANIMATION_TYPE, 0);
                    for (int index = PAGEVIEW_ANIMATION_FIRST; index <= PAGEVIEW_ANIMATION_LAST; index++) {
                        final TextView tv = new TextView(Launcher.this);
                        tv.setCompoundDrawablesWithIntrinsicBounds(null,
                                drawableTop, null, null);
                        tv.setCompoundDrawablePadding(15);

                        tv.setTag(index);
                        if (index == currentAnimationId) {
                        ...
                        tv.setOnClickListener(new OnClickListener() {
                            ...
                            public void onClick(View v) {
                                ...
                                if (mPreviousTV != null) {
                                    mPreviousTV.setTextColor(Color.WHITE);
                                }
                                ((TextView) v).setTextColor(Color.GREEN);
                                mPreviousTV = (TextView) v;
                            }
                        });
                        tv.setText("动画" + (index + 1));
                        tv.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 12);
                        LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(
                                LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT);
                        switch_animtaion_list.addView(tv, lp);
                    }
                }

//                int id = (Integer)(v.getTag());
//                id++;
//                if(id > PAGEVIEW_ANIMATION_LAST){
//                    id = PAGEVIEW_ANIMATION_FIRST;
//                }
//                v.setTag(id);
//                //设置切换动画类型
//                PageViewAnimation.getInstance().setPageViewAnime(id);
...
//                    ((TextView) v).setText(PageViewAnimation.getPageVIewAnimationName(id));
//                }
            }
        });
        switchAnimation.setVisibility(View.VISIBLE);
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            } else if("mss.action.windowsplit".equals(action)){
                FrameLayout decorView = (FrameLayout)getWindow().getDecorView();
                int orientation = getResources().getConfiguration().orientation;
                int width = getResources().getDisplayMetrics().widthPixels;
                int height = getResources().getDisplayMetrics().heightPixels;
                Log.d(TAG, "windowsplit orientation:"+orientation+ " width:"+width + " height:"+height);
                int paddingLeft = decorView.getPaddingLeft();
                int paddingRight = decorView.getPaddingRight();
                int paddingTop = decorView.getPaddingTop();
                int paddingBottom = decorView.getPaddingBottom();
                Log.d(TAG, "windowsplit paddingRight:"+paddingRight+ " paddingBottom:"+paddingBottom);
                if(orientation == Configuration.ORIENTATION_LANDSCAPE) {
                    decorView.setPadding(paddingLeft, paddingTop, (int)(width/2), paddingBottom);
                }else{
                    decorView.setPadding(paddingLeft, paddingTop, paddingRight, (int)(height/2));
                }
            }
        }
    };
    ...
    public void onAttachedToWindow() {
        ...
        filter.addAction("mss.action.windowsplit");
    ...
    public void onRestoreInstanceState(Bundle state) {
        /// M: resolve CR ALPS02266122 @{
        try{
            super.onRestoreInstanceState(state);
        }catch(Exception e){
            Log.d(TAG,"onRestoreInstanceState try exception");
        }
        state = null;
        /// @}
    ...
    public void bindAppsAdded(final ArrayList<Long> newScreens,
        ...
        if (addedApps != null && mAppsView != null) {
            ...
            if (mSupportEditAndHideApps && (sUsePendingAppsQueue || !mBindingAppsFinished
                /* op09 || !mAppsCustomizeContent.isDataReady()*/)) {
                if (LauncherLog.DEBUG) {
    ...
    public void bindAllPackages(final WidgetsModel model) {
        if (LauncherLog.DEBUG) {
            LauncherLog.d(TAG, "bindAllPackages: sUsePendingAppsQueue = "
                + sUsePendingAppsQueue);
        }
   ...
   /**
    ...
    public static boolean isInEditMode() {
        ...
        //return mAppsCustomizeTabHost.mIsInEditMode;
    }
     ...
     */
    public static class PendingChangedApplications { //op09 private
